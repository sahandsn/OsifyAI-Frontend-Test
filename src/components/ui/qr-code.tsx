...
function QRCodeSkeleton(props: QRCodeSkeletonProps) {
  const { asChild, className, style, forceMount, ...skeletonProps } = props;
  ...
  const isLoaded = (dataUrl || svgString || generationKey) && !forceMount;
  ...
  return (
    <SkeletonPrimitive
      ...
      {...skeletonProps}
      className={cn(
        "absolute max-h-(--qr-code-size) max-w-(--qr-code-size) bg-accent",
        className,
      )}
      style={{
        width: context.size,
